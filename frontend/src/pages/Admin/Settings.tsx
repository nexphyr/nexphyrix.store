...
const Settings = () => {
  ...
  return (
    <div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow overflow-hidden max-w-2xl p-6 border border-gray-100 dark:border-gray-800">
        ...
        <div className="space-y-4 max-w-md bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
          <div>
            <label className="label text-gray-500 dark:text-gray-400">Email Admin Terdaftar</label>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow overflow-hidden max-w-2xl p-6 mt-6 border border-gray-100 dark:border-gray-800">
        <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-gray-800 dark:text-gray-100">
        ...
        <div className="space-y-4 max-w-md bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
          <div>
            <label className="label text-gray-700 dark:text-gray-200 font-bold mb-2">Durasi Kedaluwarsa Pesanan (Menit)</label>
            <div className="flex items-center gap-3">
              <input 
                type="number" 
                min="1"
                className="input max-w-[150px] bg-white dark:bg-gray-900 border-gray-300 dark:border-gray-600 dark:text-white font-bold text-lg"
                value={expiryMinutes}
                onChange={(e) => setExpiryMinutes(parseInt(e.target.value) || 15)}
              />
              <span className="text-gray-500 dark:text-gray-400 font-medium">Menit</span>
            </div>
          ...
          {message && (
            <div className={`p-3 rounded-lg text-sm font-medium ${message.type === 'success' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300'}`}>
              {message.text}
            </div>
          )}
